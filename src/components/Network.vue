...
class Network extends Vue {
    ...
    private destroyed() {
        if (!NetworkClient.hasDefaultInstance()) return;
    ...
    private async _getNetworkClient(): Promise<NetworkClient> {
        if (!NetworkClient.hasDefaultInstance()) {
            const networkClient = new NetworkClient();
            networkClient.setAsDefaultInstance();
    ...
    private _fireInitialEvents() {
        ...
        if (networkClient.consensus === 'syncing') this.$emit(Network.Events.CONSENSUS_SYNCING);
        else if (networkClient.consensus === 'established') this.$emit(Network.Events.CONSENSUS_ESTABLISHED);
        else if (networkClient.consensus === 'lost') this.$emit(Network.Events.CONSENSUS_LOST);
